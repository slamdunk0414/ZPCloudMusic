...
- (void)checkPause{
    if (DownloadStatusPaused==self.downloadInfo.status || DownloadStatusNone==self.downloadInfo.status) {
...
//    self.fileHandle = [NSFileHandle fileHandleForWritingAtPath:self.downloadInfo.cachePath];
    ...
    if (![manager fileExistsAtPath:[self.downloadInfo absolutePath]]) {
        //截取文件夹路径
        NSString *downloadFolder=[[self.downloadInfo absolutePath] stringByDeletingLastPathComponent];
        
        //创建文件夹
        [manager createDirectoryAtPath:downloadFolder withIntermediateDirectories:YES attributes:nil error:nil];
        
        //创建文件
        [manager createFileAtPath:[self.downloadInfo absolutePath] contents:nil attributes:nil];
    }
    
    //创建文件句柄
    self.fileHandle = [NSFileHandle fileHandleForWritingAtPath:[self.downloadInfo absolutePath]];
